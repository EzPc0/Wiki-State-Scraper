
const axios = require ('axios');
const cheerio = require('cheerio');

const page_url = "https://en.wikipedia.org/wiki/List_of_states_and_territories_of_the_United_States"

async function getUSStates() {
	const {data} = await axios.get (page_url);
	// waits for html to return 

	const $ = cheerio.load(data);
	const table = $('caption:contains("States of the United States of America")').parent();
	// uses cheerio to search for table through jquery

	const states = [];
	table.find('tbody tr').slice(2).each((i, element) => {
		const $row =$(element);
		const state = {};
		// store state row as an array

		
		state.name = $row.find('th a').first().text().trim();
		const labels = ['code', 'captial', 'largest', 'ratification', 'popualtion', 'total_area_miles', 'total_area_km', 'land_area_miles', 'land_area_km', 'water_area_miles', 'water_area_km','number_of_reps',]
		
		let offset  = 0;
		$row.find('td').each((j, col) => {
			const $col = $(col);
			let value = $col.text().trim();
			let numValue = Number(value.replace(/,/g,''));
				if (!isNaN(numValue)) {
				value = numValue;
			}
			// let numValue = parseInt( Number(value.replace(/,/g,'')) );
			if (j === 1 && $col.attr('colspan') === '2') {
				const label = labels[j];
				state [label] = value;
				offset = 1;

			} 
			const label = labels[j + offset];
			state[label] = value; 
		});


		states.push(state);

	});

	console.log(states);
	// pulls the the rows in table
//console.log(states);


}







getUSStates();